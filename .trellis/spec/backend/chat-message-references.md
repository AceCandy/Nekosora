...
- `/api/chat` tests also keep the failure fallback conversation update permanently pending, advance the shared 5-second budget, and assert the stream reaches failed-run finalization and closes without `[DONE]`; the test must release the underlying deferred afterward because application timeout does not cancel the query.
