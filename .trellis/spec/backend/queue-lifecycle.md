...
- Core exposes a process-local Queue provider through a `globalThis` slot keyed by `Symbol.for`, so Next Node instrumentation and separately compiled server chunks share the same provider. A module-local variable is forbidden because Turbopack may emit independent module instances and optimize the instrumentation setter into a no-op. Gateway and Worker inject `getQueue` during startup; Web injects a lazy provider from Node instrumentation so browser and Edge bundles never load the driver. Web and Gateway only call `send()`; only Worker registers `work()`. Upload preserves its synchronous fallback when acquisition fails. Title/memory dispatch must acquire the provider before claiming a due durable row so a producer failure cannot postpone Worker recovery by 15 minutes.
...
- Provider regressions prove missing provider rejection occurs before title/memory DB claim and configured providers survive a module reload. Runtime tests assert resource close follows Queue drain and repeated signals still exit once.
...
- Inspect the Web production chunks to verify `configureQueueProvider` still writes the shared `Symbol.for` slot and request chunks read the same slot; a bundled empty setter is a release blocker.
