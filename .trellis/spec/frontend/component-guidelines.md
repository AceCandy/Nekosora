...
- 系统设置仅保留点击保存后立即生效；不提供草稿、修改记录、审核发布或回滚入口，也不保留对应后台接口与历史存储。`settings-control` 的 revision 只用于并发校验和缓存代际，不作为用户可见的版本历史。
...
- **固定控件与滚动列表的右边界必须用真实浏览器验收**:`overflow-y-auto` 的滚动条会占用 `clientWidth`,因此按钮在滚动区外、列表项在滚动区内时,仅比较 Tailwind class 或无占宽滚动条的几何夹具不能证明对齐。首屏宽度不得依赖水合后的 `useLayoutEffect` / `ResizeObserver` 修正,否则刷新时会先窄后宽;应让列表内容通过 CSS 尺寸容器继承固定控件宽度,滚动容器只伸入侧栏 padding 承载滚动条。用足够数据触发纵向滚动后,在 `320/390/768/1280px` 视口分别比较新对话按钮、分组标题与会话项的 `getBoundingClientRect().right`,差值须不超过 `1px`,且页面不得产生横向滚动;不要缩窄固定按钮或永久隐藏滚动条。
