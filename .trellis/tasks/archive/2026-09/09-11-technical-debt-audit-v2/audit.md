...
## 实施顺序与决策
先 D6 补回归执行入口，再 D1/D2 请求边界，然后 D3 路由类型，最后 D4/D5 Web 读取类型。每批独立验证与复核。拒绝错误输入会改变此前偶然接受的非法请求行为：建议 HTTP 400、MCP -32600/-32602，合法 n 继续 1–4 截断，数字字符串/小数 n 拒绝；用户已于 2026-09-11 确认，四批实现及验证完成，工作提交已完成；归档见任务状态。
