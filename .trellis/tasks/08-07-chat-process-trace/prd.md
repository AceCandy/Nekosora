...
- 单一面板将真实事件投影为面向用户的“理解、上下文、分析、搜索、阅读”；进入 `answering` 时必须在首个正文 token 展示前切为“研究完成”，研究耗时固定为 `startedAt -> firstContentAt`，正文流式期间不再变化，也不展示用户已能感知的答案生成阶段。Prompt 构建、隐藏推理、原始工具名/参数和 provider 尝试路径不得直接展示。
