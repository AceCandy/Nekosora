...
export default function KnowledgeDebug({ kbIds }: { kbIds: string[] }) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-xs font-semibold uppercase tracking-wider text-neutral-400">{t("debugTitle")}</div>
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-md border border-neutral-200 dark:border-neutral-800 bg-transparent px-3 py-2 text-sm focus:outline-none focus:border-sora-blue"
        />
        <button
          onClick={handleSearch}
          disabled={loading || !query.trim()}
          className="inline-flex items-center gap-1.5 rounded-md bg-sora-blue hover:bg-sora-blue-hover text-white px-4 py-2 text-sm font-semibold disabled:opacity-40 cursor-pointer"
        >
      ...
      <div className="space-y-2">
        {chunks.map((c, i) => (
          <div key={i} className="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#0d0f14] p-3">
            ...
            <p className="text-xs text-neutral-600 dark:text-neutral-400 line-clamp-4">{c.content}</p>
          </div>
        ))}
      </div>
    </div>
