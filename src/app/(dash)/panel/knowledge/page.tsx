  ...
  return (
    <div className="space-y-6">
      ...
      <form action={handleCreate} className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#0d0f14] p-4 space-y-3">
        <div className="text-xs font-semibold uppercase tracking-wider text-neutral-400">{t("createTitle")}</div>
        <input
          name="name"
      ...
      <div className="space-y-3">
        {kbs.length === 0 ? (
          <p className="text-xs text-neutral-400 py-8 text-center border border-dashed border-neutral-200 dark:border-neutral-800 rounded-lg">{t("empty")}</p>
        ) : (
          kbs.map((kb) => (
            <div key={kb.id} className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#0d0f14] p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-semibold text-sm text-neutral-900 dark:text-white">{kb.name}</div>
                  {kb.description && <div className="text-xs text-neutral-500 mt-0.5">{kb.description}</div>}
