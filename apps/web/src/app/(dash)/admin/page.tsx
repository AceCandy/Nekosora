      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((s) => (
          <Link
            key={s.labelKey}
            href={s.href}
            className="group block rounded-lg border border-morning-mist bg-white p-5 hover:border-sora-blue/30 hover:bg-neutral-50/50     transition-[background-color,border-color,box-shadow] duration-200 shadow-none hover:shadow-[0_4px_12px_rgba(0,0,0,0.03)]"
          >
            <div className="text-ui-caption font-medium text-neutral-400  group-hover:text-sora-blue transition-colors">{t(s.labelKey)}</div>
            <div className="mt-2 text-ui-display font-semibold tracking-tight text-neutral-900  font-mono">{s.value}</div>
            <div className="mt-1 text-ui-caption text-neutral-400 ">{t(s.subKey, { count: s.subCount })}</div>
          </Link>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {quickLinks.map((q) => (
            <Link
              key={q.href}
              href={q.href}
              className="block rounded-lg border border-morning-mist bg-white p-5 hover:border-sora-blue/30 hover:bg-neutral-50/50     transition-[background-color,border-color,box-shadow] duration-200 shadow-none hover:shadow-[0_4px_12px_rgba(0,0,0,0.03)]"
            >
              <div className="text-ui-body font-semibold text-neutral-800 ">{t(q.titleKey)}</div>
              <div className="mt-1 text-ui-caption text-ink-tertiary leading-relaxed">{t(q.descKey)}</div>
            </Link>
          ))}
        </div>
      </div>
