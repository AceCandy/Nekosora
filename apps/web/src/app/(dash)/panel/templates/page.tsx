...
export default async function PanelTemplatesPage() {
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      {templates.length === 0 && (
        <div className="rounded-lg border border-neutral-200  bg-white  p-10 text-center shadow-none">
        ...
        <div key={category} className="space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {items.map((t) => (
              <div
                key={t.id}
                className="rounded-lg border border-neutral-200 bg-white   p-4 shadow-none hover:border-sora-blue/40 transition-colors duration-150"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-ui-title">{t.icon ?? "📄"}</span>
                    <div>
                      <div className="font-semibold text-ui-body text-neutral-900  flex items-center gap-1.5">
                        {t.name}
                        {t.isAgent && (
                ...
                {t.description && (
                  <p className="mt-2 text-ui-caption text-neutral-500  leading-relaxed line-clamp-2">{t.description}</p>
                )}
