/**
 * 存储工厂 —— 按 STORAGE_DRIVER 环境变量选后端,惰性初始化单例。
 *
 * 选择逻辑(对标 db/index.ts 的 resolveDialect):
 *   1. STORAGE_DRIVER=s3/r2/minio + 完整配置 → S3Driver(动态 import aws-sdk)
...
export function resolveStorageKind(): S3LikeKind | null {
  ...
  throw new Error("STORAGE_DRIVER 仅允许 local、s3、r2 或 minio");
}

let _storage: StorageDriver | null = null;
let _resolvedKind: StorageKind = "local";

/** 构造 S3 driver；配置不全或构造失败时直接抛错。 */
