...
export async function POST(req: Request) {
  ...
  if (body.parentPublicId) {
    ...
    if (!parent) {
      return Response.json({ error: "父消息不存在或不属于当前会话" }, { status: 400 });
    }
  ...
  if (body.sourcePublicId) {
    ...
    if (!source) {
      return Response.json({ error: "源消息不存在或不属于当前会话" }, { status: 400 });
    }
  ...
  if (body.continueFromPublicId) {
    ...
    if (!contMsg) {
      return Response.json({ error: "续写消息不存在或不属于该会话" }, { status: 400 });
    }
    if (contMsg.role !== "assistant") {
      return Response.json({ error: "仅支持在 assistant 消息上续写" }, { status: 400 });
    }
    isContinue = true;
    ...
    if (!continueParentUserPublicId) {
      return Response.json({ error: "续写消息缺少当前会话内的用户父消息" }, { status: 400 });
    }
  }
  ...
  if (isContinue) {
    if (!continueParentUserPublicId) {
      return Response.json({ error: "续写消息缺少用户父消息" }, { status: 400 });
    }
    ...
    if (!userMessage || userMessage.role !== "user") {
      return Response.json({ error: "用户消息不存在或不属于当前会话" }, { status: 400 });
    }
    ...
    if (!userContent.trim() && messageAttachments.length === 0) {
      return Response.json({ error: "消息内容和图片不能同时为空" }, { status: 400 });
    }
